...
//player class used for determining location and direction of player
	class SpacePlayer{
		 ...
		 */
		public SpacePlayer(int x, int y) {
			this.x = x;
			this.y = y;
			dead = false;
		}
		...
		public void drawPlayer(Graphics g) {
			img = SpaceImages.getShip(); //get image from Images class
	...
	class SpaceMap extends JPanel implements ActionListener{
		private int difficulty;
		private SpacePlayer player;//player object
		...
		private SpaceObstacle[] arr, arr2, arr3;//array containing obstacles
		 ...
		 */
		public SpaceMap() throws Exception{
			lblPause = new JLabel("PAUSED");
			lblStart = new JLabel("PRESS SPACE TO START");
			lblScore = new JLabel("SCORE: 0");
			//load images for later use
			SpaceImages.loadImages();
			//instantiate player
			player = new SpacePlayer(20,350);
			score = 0;// initialize score to 0
			arr = new SpaceObstacle[20];//initialize Obstacle array
			arr2 = new SpaceObstacle[20];
			arr3 = new SpaceObstacle[20];
		...
		public void actionPerformed(ActionEvent e) {
			if(!paused) {
				...
				if(e.getSource()==obsTimer) {//if obstacle timer goes off
					arr[cnt%20] = new SpaceObstacle(1300, SpaceObstacle.genInt());//spawn new obstacle at random y-coordinate
					cnt++;//increment cnt
					if(formCnt!=20) {//keep formCnt under 10
						formCnt++;
					}
					if(difficulty>=1) {
						arr2[cnt2%20] = new SpaceObstacle(1400, SpaceObstacle.genInt());
						cnt2++;
						if(formCnt2!=20) {
							formCnt2++;
						}
					...
					if(difficulty>=2) {
						arr3[cnt3%20] = new SpaceObstacle(1500, SpaceObstacle.genInt());
						cnt3++;
						if(formCnt3!=20) {
							formCnt3++;
						}
		...
		public void paintComponent(Graphics g) {
			try {
				...
				//draw background
				g.drawImage(SpaceImages.getBackground(), 0, 0, null);
				//draw player
				player.drawPlayer(g);
				//draw obstacles
		...
		class MyKeyListener extends KeyAdapter{//class to determine key events
			public void keyPressed(KeyEvent e) {
				if(e.getKeyCode()==32) {
					if(!SpaceMap.getStarted()) {
						SpaceMap.setStarted(true);
					}
					if(SpaceMap.getPaused()==true) {
						SpaceMap.setPaused(false);
					}
					else {
						SpaceMap.setPaused(true);
					}
				}
	...
	class SpaceObstacle{//class for rockets
		//current location
		private int x;
		private int y;
		 ...
		 */
		public SpaceObstacle(int x, int y) {
			this.x = x;
			this.y = y;
		}
		//getter and setter methods
	...
	class SpaceImages{//class containing methods to access images
