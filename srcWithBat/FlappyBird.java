...
class TopObstacle{
	...
	public void drawObstacle(Graphics g) {
		g.drawImage(FlappyImages.getTop(), x, y, null);
	}
	/**
...
class BotObstacle{
	 ...
	 */
	public void drawObstacle(Graphics g) {
		g.drawImage(FlappyImages.getBot(), x, y, null);
	}
}
class FlappyPlayer{
	private int x, y, dy;
	 ...
	 * @param y starting y-coordinate
	 */
	public FlappyPlayer(int x, int y) {
		this.x = x;
		this.y = y;
		this.dy = 30;
		dead = false;
	...
	public void drawPlayer(Graphics g) {
		img = FlappyImages.getFlappy();
		g.drawImage(img, x, y, null);
	}

}
class FlappyMap extends JPanel implements ActionListener{
	 ...
	 */
	public FlappyMap() throws Exception {
		//initialize arrays
		...
		//instantiate player
		player = new FlappyPlayer(50,300);
		//load images
		FlappyImages.loadImages();
	...
	class MyKeyListener extends KeyAdapter{//class to determine key events
		public void keyPressed(KeyEvent e) {
			if(e.getKeyChar()=='p') {
				if(FlappyMap.getPaused()==true) {
					FlappyMap.setPaused(false);
				}
				else {
					FlappyMap.setPaused(true);
				}
			}
			else if(e.getKeyChar() == ' ') {//space
				if(!FlappyMap.getStarted()) {
					FlappyMap.setStarted(true);
				}
				player.setDy(20);
				repaint();
			}
	...
	public void paintComponent(Graphics g) {
		try {
			...
			g.drawImage(FlappyImages.getBackground(), 0, 0, null);
			for (int i = 0; i<formCnt;i++) {
