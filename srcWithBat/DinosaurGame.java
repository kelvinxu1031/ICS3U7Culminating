...
public class DinosaurGame  extends JFrame{
	...
	public DinosaurGame() throws Exception {
		...
		map = new DinoMap();//instantiate JPanel
		f.add(map);
		f.setSize(1280,720);
...
/**
 * DinoPlayer class to set position of the player
 * @author Kelvin Xu
 *
 */
class DinoPlayer{
	...
	public void myDraw(Graphics g) throws Exception{
		if(jumping) {//display jumping image
			img = DinoImages.getJump();
		}
		else if(dead) {//display dead image
			img = DinoImages.getDead();

		}
		else if(running) {//display running images
			if(cnt == 0) {
				img = DinoImages.getRun()[cnt];
				cnt=1;
			}
			else {
				img = DinoImages.getRun()[cnt];
...
/**
 * DinoMap class to create JPanel containing the game
 * @author Kelvin Xu
 *
 */
class DinoMap extends JPanel implements ActionListener{
	...
	/**
	 * Constructor for DinoMap class
	 */
	public DinoMap() throws Exception {
		//initialize arrays
		arr = new DinoObstacle[10];
		picCnt = new int[10];
		//set DinoObstacle settings
		DinoObstacle.setDx(25);
		//set score and count to 0
		score = 0;
		tickCnt = 0;
		//instantiate player
		player = new DinoPlayer(100,530);
		//load images for future use
		DinoImages.loadImages();
	...
	public void actionPerformed(ActionEvent e) {
		if(!paused) {//if paused: nothing gets updated(maintain the position of everything)
			if(e.getSource() == obsTimer) {//spawn obstacle
				DinoObstacle obstacle = new DinoObstacle(2000, 536);
				arr[cnt%10] = obstacle;
			...
			if(tickCnt>500) {
				DinoObstacle.setDx(50);//level 2 speed
			}
			tickCnt++;
	...
	class MyRunnerKeyListener extends KeyAdapter{//class to determine key events
		public void keyPressed(KeyEvent e) {
			if(e.getKeyChar()=='p') {//pause the game
				if(DinoMap.getPaused()==true) {
					DinoMap.setPaused(false);
				}
				else {
					DinoMap.setPaused(true);
				}
			}
				...
				if(!DinoMap.getStarted()) {
					DinoMap.setStarted(true);
				}
	...
	public void paintComponent(Graphics g) {
		try {
			...
			//draw background
			g.drawImage(DinoImages.getBackground(), 0, 0, null);
			updateLabel();
...
/**
 * DinoObstacle class to determine location of obstacles
 * @author Kelvin Xu
 *
 */
class DinoObstacle{
	...
	/**
	 * Constructor for DinoObstacle class
	 * @param x starting x-coordinate
	 * @param y starting y-coordinate
	 */
	public DinoObstacle(int x, int y) {
		this.x = x;
		this.y = y;
	}
	/**
	...
	public void drawCactus1(Graphics g) {
		g.drawImage(DinoImages.getCactus(), x, y, null);
	}
	public void drawCactus2(Graphics g) {
		g.drawImage(DinoImages.getCactus2(), x, y, null);
	}
	public void drawCactus3(Graphics g) {
		g.drawImage(DinoImages.getCactus3(), x, y, null);
	}

}

/**
 * DinoImages class to load and get images
 * @author Kelvin Xu
 *
 */
class DinoImages{
